...
const Generacion7 = () => {
  ...
  useEffect(() => {
    if (window.localStorage.getItem("pokeg7") === null) {
      // console.log("No hay nada en el localStorage asi que voy a hacer Fetch ");
      getPokemon().then((data) => {
        setPokeg7(data);
        window.localStorage.setItem("pokeg7", JSON.stringify(data));
      });
    } else {
      console
        .log
        ...
        ();
      // setPokeg7(JSON.parse(localStorage.getItem("pokeg7") || "[]"));
      // setPokeg7(localStorage.getItem("pokeg7") || "[]");
      // setPokelocalStorage(JSON.parse(localStorage.getItem("pokeg7") || "[Akitoy]"));
      setPokeg7(JSON.parse(localStorage.getItem("pokeg7") || "[Akitoy]"));
      //Gurdar sola la imgaen en el localStorage
      // console.log(localStorage.getItem("pokeg7"));

      //Obtener el valor de pokeg7 del localStorage
      //  console.log(pokelocalStorage.results);

      // (localStorage.getItem("pokeg7") || "[]").results.map((pokemon: any) => {
  ...
  const handleDeleteLocalStorage = () => {
    // localStorage.removeItem("pokeg7");
  ...
  const filtratPokemon = (busqueda: string): void => {
    ...
    setPokeg7({ ...pokeg7, results: resultado });
  };
    ...
    <div className=" ">
      ...
      <form className="md:col-span-4 flex items-center justify-center gap-2 mt-10">
        <input
          ...
          placeholder="Buscar un pokemon de la septima generación"
        />
        <svg
