...
const Generacion3 = () => {
  ...
  useEffect(() => {
    if (window.localStorage.getItem("pokeg3") === null) {
      // console.log("No hay nada en el localStorage asi que voy a hacer Fetch ");
      getPokemon().then((data) => {
        setPokeg3(data);
        window.localStorage.setItem("pokeg3", JSON.stringify(data));
      });
    } else {
      console
        .log
        ...
        ();
      // setPokeg3(JSON.parse(localStorage.getItem("pokeg3") || "[]"));
      // setPokeg3(localStorage.getItem("pokeg3") || "[]");
      // setPokelocalStorage(JSON.parse(localStorage.getItem("pokeg3") || "[Akitoy]"));
      setPokeg3(JSON.parse(localStorage.getItem("pokeg3") || "[Akitoy]"));
      //Gurdar sola la imgaen en el localStorage
      // console.log(localStorage.getItem("pokeg3"));

      //Obtener el valor de pokeg3 del localStorage
      //  console.log(pokelocalStorage.results);

      // (localStorage.getItem("pokeg3") || "[]").results.map((pokemon: any) => {
  ...
  const handleDeleteLocalStorage = () => {
    // localStorage.removeItem("pokeg3");
  ...
  const filtratPokemon = (busqueda: string): void => {
    ...
    setPokeg3({ ...pokeg3, results: resultado });
  };
    ...
    <div className=" ">
      ...
      <form className="md:col-span-4 flex items-center justify-center gap-2 mt-10">
        <input
          ...
          placeholder="Buscar un pokemon de la tercera generación"
        />
        <svg
