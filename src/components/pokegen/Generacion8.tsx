...
const Generacion8 = () => {
  ...
  useEffect(() => {
    if (window.localStorage.getItem("pokeg8") === null) {
      // console.log("No hay nada en el localStorage asi que voy a hacer Fetch ");
      getPokemon().then((data) => {
        setPokeg8(data);
        window.localStorage.setItem("pokeg8", JSON.stringify(data));
      });
    } else {
      console
        .log
        ...
        ();
      // setPokeg8(JSON.parse(localStorage.getItem("pokeg8") || "[]"));
      // setPokeg8(localStorage.getItem("pokeg8") || "[]");
      // setPokelocalStorage(JSON.parse(localStorage.getItem("pokeg8") || "[Akitoy]"));
      setPokeg8(JSON.parse(localStorage.getItem("pokeg8") || "[Akitoy]"));
      //Gurdar sola la imgaen en el localStorage
      // console.log(localStorage.getItem("pokeg8"));

      //Obtener el valor de pokeg8 del localStorage
      //  console.log(pokelocalStorage.results);

      // (localStorage.getItem("pokeg8") || "[]").results.map((pokemon: any) => {
  ...
  const handleDeleteLocalStorage = () => {
    // localStorage.removeItem("pokeg8");
  ...
  const filtratPokemon = (busqueda: string): void => {
    ...
    setPokeg8({ ...pokeg8, results: resultado });
  };
    ...
    <div className=" ">
      ...
      <form className="md:col-span-4 flex items-center justify-center gap-2 mt-10">
        <input
          ...
          placeholder="Buscar un pokemon de la octava generación"
        />
        <svg
